= HL7 Module
:keywords: b2b, hl7, schema, module, edi

The HL7 Module provides you with the ability to convert HL7 ER7 messages to and from DataWeave-compatible representations using lists and maps.

The HL7 Module includes:
...
. Configure the HL7 Module for your trading partner according to your implementation convention.
. Use the HL7 module inside Mule flows.
...
=== Installing the HL7 Module in Anypoint Studio

Follow the steps below to install an HL7 Module connector in Anypoint Studio.

NOTE: To use the HL7 Module connector in a production environment, you must have purchased a license for Anypoint B2B.

. In the *Help* menu in *Anypoint Studio*, select *Install New Software*. The *Available Software* page appears.
+
image:hl7-module-2.png[hl7-module-2]
+
...
By way of example, here's the start of a sample overlay schema modifying the basic HL7 2.5.1 ADT_A01 message structure definition. This sample customizes the `PD1` segment and specifies that it is mandatory, so an error will be reported if the segment is not present in a receive or send message.
...
The modifications in this example specify that the PD1 segment and the INSURANCE group are required in each message, and that the PROCEDURE group is unused. With this overlay, errors will be reported if the PD1 segment or the INSURANCE group is not present in a message, or if the PROCEDURE group is present.
...
|data |List of segment and group modifications withinthe structure (optional, each is only used when there are modifications to that section).
|===
...
A segment overlay again details modifications to the base schema definition. Most often these modifications take the form of marking elements or composites in the base definition as unused, but any usage or repetition count change is allowed. Here a full overlay including some sample segment overlays:
...
This example modifies the base definitions for the ROL and DG1 segments. It makes the values at position 2 and 4 of the ROL segment optional (they are required in the base definition), and makes all values after the first four in the DG1 segment unused (which drops them from the metadata representation and means they'll be ignored in data).
...
Follow these steps to create a global HL7 module configuration in a Mule application:

. Click the *Global Elements* tab at the base of the canvas, then click *Create*.
. In the *Choose Global Type* wizard, use the filter to locate and select, *HL7 Module*, then click *OK*.
+
...
Follow these steps to configure a HL7 module in your application.
...
After you set up a global element for your HL7 module, configure the schemas, acknowledgments, IDs, and the parser options. A reference for these options is located in the link:http://mulesoft.github.com/edi-module[EDI module reference].

=== Setting Your Schema Locations

NOTE: Currently, you can only configure schema locations in the Anypoint Studio XML view.

Using the schema locations determined above, switch to the XML view by clicking *Configuration XML* in Studio and modify your HL7 module configuration to include a list of all the schemas you wish to include by adding an <http://edischema[edi:schema]> element for each document type:
...
You can configure the MSH application and facility identification for you and your trading partner on the HL7 Module connector configuration.
